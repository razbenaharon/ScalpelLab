...
class multiMPV:
    ...
    def get_vids(self):
        ...
        while True:
            ...
            files = askopenfilenames(initialdir=last_dir, title='select video', filetypes=self.__filetypes)

            if not files:
                # User cancelled the dialog
                break

            # Remember the last directory for convenience
            if files:
            ...
            if len(vids) >= 9:
                messagebox.showinfo(title="Maximum Reached",
                                   message=f"You've selected {len(vids)} videos. Maximum is 9.")
                break

            # Ask if user wants to add more videos from another folder
            add_more = messagebox.askyesno(title="Add More Videos?",
                                          message=f"Currently selected: {len(vids)} video(s).\n\nDo you want to add more videos from another folder?")
            if not add_more:
                break
