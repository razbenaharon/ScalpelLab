"""
Multi-Person Pose Detection using YOLO26 Pose with BoxMOT BoT-SORT + OSNet ReID

This script uses the BoxMOT library's BoT-SORT tracker with external OSNet ReID model
for superior re-identification after occlusions compared to Ultralytics native tracker.

USAGE:
    python 1_pose_anesthesiologist_BotSort.py [video_path] [output_path]

    If no arguments provided, uses paths from CONFIG below.

OUTPUT:
    Parquet file (*_keypoints_boxmot.parquet) with columns:
...
CONFIGURATION:
    ...
    boxmot:
        ...
        track_high_thresh  - High confidence for track confirmation (0.6)
        track_low_thresh   - Low confidence for detection (0.1)
        new_track_thresh   - Threshold for new tracks (0.7)
        track_buffer       - Frames to keep lost tracks (300 = 10s at 30fps)
        match_thresh       - Association threshold (0.5)
        proximity_thresh   - IoU proximity threshold (0.5)
        appearance_thresh  - Cosine distance for appearance (0.15, lower = stricter)
        cmc_method         - Camera motion compensation (sof, orb, ecc, sift)

REQUIREMENTS:
    pip install ultralytics opencv-python numpy pandas pyarrow boxmot torch

COMPARISON WITH OTHER TRACKERS:
    - vs Ultralytics BoT-SORT: Better ReID with OSNet, more tunable parameters
    - vs StrongSort: Faster, but slightly less robust for long occlusions
    - Best for: Medium complexity scenes with moderate occlusions
"""
...
def pose_anesthesiologist_boxmot(video_path, output_path=None):
    """
    Detect and track ALL persons from video using YOLO26 pose detection with BoxMOT BoT-SORT + OSNet tracking.
    ...
    reid_model_path = os.path.join(
        CONFIG['boxmot']['reid_model_dir'],
        CONFIG['boxmot']['reid_model']
    )
    ...
    tracker = BotSort(
        ...
        track_high_thresh=CONFIG['boxmot']['track_high_thresh'],
        track_low_thresh=CONFIG['boxmot']['track_low_thresh'],
        new_track_thresh=CONFIG['boxmot']['new_track_thresh'],
        track_buffer=CONFIG['boxmot']['track_buffer'],
        match_thresh=CONFIG['boxmot']['match_thresh'],
        proximity_thresh=CONFIG['boxmot']['proximity_thresh'],
        appearance_thresh=CONFIG['boxmot']['appearance_thresh'],
        cmc_method=CONFIG['boxmot']['cmc_method'],  # Available: ecc, orb, sof, sift
        with_reid=True
    )

    print("BoxMOT BoT-SORT tracker initialized!")
    print("=" * 70)
    ...
    parquet_path = output_path.replace('.mp4', '_keypoints_boxmot.parquet')
    ...
    try:
        ...
        print(f"Tracker: BoxMOT BoT-SORT")
        print(f"ReID Model: {CONFIG['boxmot']['reid_model']}")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("MULTI-PERSON POSE DETECTION - YOLO26-Pose + BoxMOT BoT-SORT")
    print("=" * 70)
    print(f"Model: {CONFIG['yolo']['model']}")
    print(f"Tracker: BoxMOT BoT-SORT")
    print(f"ReID Model: {CONFIG['boxmot']['reid_model']}")
    ...
    # Run pose detection
    try:
        result_path = pose_anesthesiologist_boxmot(video_path, output_path)
        print("\nSUCCESS!")
